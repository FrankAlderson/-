...
namespace DIPLOM_V2
{
    public partial class Studenti_ugltu_dobavit : Form
    {
        SqlConnection sqlConnection;
        public Studenti_ugltu_dobavit()
        {
            InitializeComponent();
        }
    ...
    private void Studenti_ugltu_dobavit_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "diplomDataSet.Студенты_УГЛТУ". При необходимости она может быть перемещена или удалена.
            //this.студенты_УГЛТУTableAdapter.Fill(this.diplomDataSet.Студенты_УГЛТУ);
            ...
            // TODO: данная строка кода позволяет загрузить данные в таблицу "diplomDataSet.Группы". При необходимости она может быть перемещена или удалена.
            this.группыTableAdapter.Fill(this.diplomDataSet.Группы);
        ...
        private async void buttonInsert_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand command = new SqlCommand("INSERT INTO [Студенты_УГЛТУ] (Фамилия, Имя, Отчество, [Дата рождения], Институт, Группа, Курс, Пол, [Физ.Отклонения], Рейтинг, [Супруг/Супруга], Дети) VALUES (@Фамилия, @Имя, @Отчество, @Дата_рождения, @Институт, @Группа, @Курс, @Пол, @Физ_Отклонения, @Рейтинг, @Cупруги, @Дети)", sqlConnection);
            ...
            nomerinst = comboBoxInst.SelectedIndex + 1;
            command.Parameters.AddWithValue("Институт", nomerinst);
            ...
            {
                command.Parameters.AddWithValue("Физ_Отклонения", 0);
            }
            else
            {
                command.Parameters.AddWithValue("Физ_Отклонения", 1);
            }

            if (textBoxRating.Text == "0")
            {
                rating = 0;
                command.Parameters.AddWithValue("Рейтинг", rating);
            }
            else
            {

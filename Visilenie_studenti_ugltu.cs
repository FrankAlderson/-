...
namespace DIPLOM_V2
{
    public partial class Visilenie_studenti_ugltu : Form
    {
        SqlConnection sqlConnection;

        public Visilenie_studenti_ugltu()
        {
            InitializeComponent();
            groupBoxDopInfo.Visible = false;
            groupBox1.Location = new Point(89, 165);
        }
        ...
        async void naitinomer()
        {
            ...
            if (textBoxSex.Text == "Мужчина")
            {
                ...
                button3.Text = "Выселение супруги";
            }
            else if (textBoxSex.Text == "Женщина")
            {
                groupBoxSupr.Text = "Супруг";
                button3.Text = "Выселение супруга";
            }
            ...
            try
            {
                SqlCommand command16 = new SqlCommand("SELECT Номер_общежития FROM Общежития WHERE Номер_студента_углту = @Номер_студента", sqlConnection);
            ...
            try
            {
                SqlCommand command17 = new SqlCommand("SELECT Блок FROM Общежития WHERE Номер_студента_углту = @Номер_студента", sqlConnection);
            ...
            try
            {
                SqlCommand command18 = new SqlCommand("SELECT Комната FROM Общежития WHERE Номер_студента_углту = @Номер_студента", sqlConnection);
            ...
            try
            {
                SqlCommand command19 = new SqlCommand("SELECT Место FROM Общежития WHERE Номер_студента_углту = @Номер_студента", sqlConnection);
        ...
        private void visibledopinfi(object sender, EventArgs e)
        {
            if (checkBoxDopInfo.Checked == true)
            {
                ...
                groupBox1.Location = new Point(89, 331);
            }
            else
            {
                groupBoxDopInfo.Visible = false;
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                if (result2 == DialogResult.Yes)
                {
                    ...
                    SqlCommand command = new SqlCommand("UPDATE Общежития SET Номер_студента_углту = 0, Дата_заселения = NULL, Статус = 'Свободно', Дата_выселения = NULL WHERE Номер_общежития = @Obsh AND Блок = @Blok AND Комната = @Komnata AND Место = @Mesto", sqlConnection);
